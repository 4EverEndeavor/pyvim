...
from ..cursor_shapes import AnyCursorShapeConfig, to_cursor_shape_config
...
from ..eventloop.utils import call_soon_threadsafe
from ..filters import Condition, Filter, FilterOrBool, to_filter
from ..formatted_text import AnyFormattedText
from ..input.base import Input
from ..input.typeahead import get_typeahead, store_typeahead
from ..key_binding.bindings.page_navigation import (
    load_page_navigation_bindings,
)
from ..key_binding.defaults import load_key_bindings
from ..key_binding.emacs_state import EmacsState
...
from ..key_binding.key_processor import KeyPressEvent, KeyProcessor
...
from ..layout.dummy import create_dummy_layout
from ..layout.layout import Layout, walk
from ..output import ColorDepth, Output
...
from ..utils import Event, in_main_thread
...
class Application(Generic[_AppResult]):
    ...
    :param layout: A :class:`~toolkit.layout.Layout` instance.
    :param key_bindings:
        :class:`~toolkit.key_binding.KeyBindingsBase` instance for
        the key bindings.
    :param clipboard: :class:`~toolkit.clipboard.Clipboard` to use.
    ...
    :param mouse_support: (:class:`~toolkit.filters.Filter` or
        boolean). When True, enable mouse support.
    :param paste_mode: :class:`~toolkit.filters.Filter` or boolean.
    :param editing_mode: :class:`~toolkit.enums.EditingMode`.
    ...
    :class:`~toolkit.application.Application` object as input.)
    ...
    :param input: :class:`~toolkit.input.Input` instance.
    :param output: :class:`~toolkit.output.Output` instance. (Probably
    ...
    async def run_async(
        ...
        """
        Run the toolkit :class:`~toolkit.application.Application`
        until :meth:`~toolkit.application.Application.exit` has been
        called. Return the value that was passed to
        :meth:`~toolkit.application.Application.exit`.

        This is the main entry point for a toolkit
        :class:`~toolkit.application.Application` and usually the only
    ...
    def _on_background_task_done(self, task: asyncio.Task[None]) -> None:
        ...
        if exc is not None:
            get_running_loop().call_exception_handler(
                {
                    "message": f"toolkit.Application background task {task!r} "
