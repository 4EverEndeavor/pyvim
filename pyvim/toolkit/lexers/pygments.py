"""
Adaptor classes for using Pygments lexers within ..
...
from ..formatted_text.base import StyleAndTextTuples
from ..formatted_text.utils import split_lines
from ..styles.pygments import pygments_token_to_classname
...
class _TokenCache(Dict[Tuple[str, ...], str]):
    """
    Cache that converts Pygments tokens into `.` style objects.
...
class PygmentsLexer(Lexer):
    ...
    Note: Don't forget to also load a Pygments compatible style. E.g.::
        ...
        from ..styles.from_pygments import style_from_pygments_cls
        ...
        def create_line_generator(start_lineno: int, column: int = 0) -> LineGenerator:
            ...
            def get_text_fragments() -> Iterable[tuple[str, str]]:
                ...
                for _, t, v in self.pygments_lexer.get_tokens_unprocessed(text):
                    # Turn Pygments `Token` object into . style
                    # strings.
